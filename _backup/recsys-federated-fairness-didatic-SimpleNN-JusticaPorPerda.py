...
def treinar_modelos_locais_backup(modelo_global, avaliacoes_inicial, criterion, epochs=50, learning_rate=0.01):
    """
...
def treinar_modelos_locais(modelo_global, avaliacoes_inicial, criterion, epochs=50, learning_rate=0.01):
    ...
    # print("avaliacoes_final")
    # print(avaliacoes_final)

    # Retorna ambos: avaliações finais, avaliações finais por cliente, modelos dos clientes e perdas dos modelos
    return avaliacoes_final, modelos_clientes, modelos_clientes_perdas

def treinar_modelos_locais_lis(modelo_global, avaliacoes_inicial, criterion, epochs=50, learning_rate=0.01):
    """
    Treina modelos locais para cada conjunto de avaliações de usuário e gera novas avaliações.
    ...
    for i, modelo_cliente in enumerate(modelos_clientes):
        ...
        modelos_clientes_perdas.append((i, lis_cliente[i]))
...
def main():
    ...
    # avaliacoes_final_tensor, modelos_clientes, modelos_clientes_perdas = treinar_modelos_locais(modelo_global_federado1, avaliacoes_inicial_tensor, criterion)
    avaliacoes_final_tensor, modelos_clientes, modelos_clientes_perdas = treinar_modelos_locais_lis(modelo_global_federado1, avaliacoes_inicial_tensor, criterion)
    ...
    agregar_modelos_locais_ao_global_pesos_justica_erro(modelo_global_federado2, modelos_clientes, modelos_clientes_perdas)

    treinar_modelo_global(modelo_global_nao_federado, avaliacoes_final_tensor, criterion) # Simulando um modelo não federado

    print("id(modelo_global_federado1): ", id(modelo_global_federado1))
    print("id(modelo_global_federado2): ", id(modelo_global_federado2))
    print("id(modelo_global_nao_federado): ", id(modelo_global_nao_federado))
    ...
    list_users = [i for i, _ in modelos_ordenados]
