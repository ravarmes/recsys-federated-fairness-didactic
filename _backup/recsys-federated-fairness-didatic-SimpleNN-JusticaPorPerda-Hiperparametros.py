...
def grid_search(modelo, avaliacoes, criterion, param_grid, epochs=50):
    """
    Executa pesquisa em grade para otimização de hiperparâmetros.
    ...
    Args:
        ...
        param_grid (dict): Um dicionário contendo os hiperparâmetros a serem testados.
            Exemplo: {'learning_rate': [0.001, 0.01, 0.1], 'epochs': [50, 100, 200]}
        epochs (int, optional): Número de épocas para o treinamento. Padrão é 50.

    Returns:
        dict: Um dicionário contendo os hiperparâmetros ótimos e a perda associada.
    """
    best_loss = float('inf')
    best_params = None
    ...
    for lr in param_grid['learning_rate']:
        for epoch in param_grid['epochs']:
            modelo_copy = copy.deepcopy(modelo)  # Cria uma cópia do modelo para cada combinação de hiperparâmetros
            treinar_modelo_global(modelo_copy, avaliacoes, criterion, epochs=epoch, learning_rate=lr)
            output = modelo_copy(avaliacoes)
            loss = criterion(output, avaliacoes).item()
...
def treinar_modelo_global2(modelo, avaliacoes, criterion, epochs=50, learning_rate=0.01, weight_decay=0):
    """
    Treina o modelo global usando uma matriz de avaliações, com regularização L2 e otimização de hiperparâmetros.
    ...
    Args:
        ...
        epochs (int, optional): Número de épocas para o treinamento. Padrão é 50.
        learning_rate (float, optional): Taxa de aprendizado para o otimizador SGD. Padrão é 0.01.
        weight_decay (float, optional): Coeficiente de regularização L2 (weight decay). Padrão é 0.

    Descrição:
        Esta função treina o modelo global utilizando a matriz de avaliações fornecida.
        Utiliza o otimizador SGD (Descida do Gradiente Estocástica) com a taxa de aprendizado
        especificada e a regularização L2 especificada pelo coeficiente weight_decay.
        Os parâmetros do modelo são atualizados em cada passo do treinamento para minimizar a função de perda.
    """
    optimizer = optim.SGD(modelo.parameters(), lr=learning_rate, weight_decay=weight_decay)
    ...
    for epoch in range(epochs):
        ...
        # Adicionando regularização L2 à função de perda
        l2_regularization = 0
...
def agregar_modelos_locais_ao_global_pesos_justica_erro2(modelo_global, modelos_clientes, modelos_clientes_perdas):
    """
    Atualiza os parâmetros do modelo global com a média ponderada dos parâmetros dos modelos locais,
    baseada nas perdas dos modelos locais.
    ...
    # Calcular os pesos de agregação baseados nas perdas
    # potencia = 2  # Potência para aumentar a diferença entre os pesos
    # pesos = [math.pow(perda, potencia) / total_perdas for _, perda in modelos_clientes_perdas]

    pesos = [1 / perda for _, perda in modelos_clientes_perdas]
...
def main():
    ...
    # param_grid = {'learning_rate': [0.001, 0.01, 0.1], 'epochs': [50, 100, 200, 250, 300, 400, 500]}
    # best_params = grid_search(modelo_global_federado1, avaliacoes_inicial_tensor, criterion, param_grid)
    # print("Melhores hiperparâmetros:", best_params['best_params'])
    # print("Menor perda encontrada:", best_params['best_loss'])

    treinar_modelo_global2(modelo_global_federado1, avaliacoes_inicial_tensor, criterion, 500, 0.1)
    ...
    avaliacoes_final_tensor, modelos_clientes, modelos_clientes_perdas = treinar_modelos_locais(modelo_global_federado1, avaliacoes_inicial_tensor, criterion, 500, 0.1)
    ...
    list_users = agregar_modelos_locais_ao_global_pesos_justica_erro2(modelo_global_federado2, modelos_clientes, modelos_clientes_perdas)
