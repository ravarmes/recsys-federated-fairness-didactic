...
class RecommenderGNN(torch.nn.Module):
    def __init__(self, input_feature_dim, hidden_dim, output_dim):
        super(RecommenderGNN, self).__init__()
        # As duas camadas de GraphSAGE convolução
        self.conv1 = SAGEConv(input_feature_dim, hidden_dim) 
        self.conv2 = SAGEConv(hidden_dim, output_dim)

    def forward(self, data):
        x, edge_index = data.x, data.edge_index

        # Primeira camada de convolução com ativação ReLU
        ...
        # Sigmoid para normalizar as saídas para o intervalo [0, 1]
        x = torch.sigmoid(x)
        
        # Escalando a saída de [0, 1] para [1, 5]
        x = x * 4 + 1
...
def treinar_modelos_locais(modelo_global, avaliacoes, avaliacoes_random, G, criterion, epochs=1, learning_rate=0.02):
